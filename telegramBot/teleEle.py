...
class TelegramBot(object):
    ...
    def selectedParam(self, update, context, query, keyboard_params):
        ...
        print("best: ", best)
        print("json ready for get: ", data)

        #r = requests.get(f"http://{IP_InfluxDB}:{port_InfluxDB}/db/getData", params=data)
        r = requests.get(f"http://192.168.1.215:9099/db/getData", params=data)
    ...
    def end(self, update, context):
        ...
        if query.data == 'home':
            ...
            r = requests.get(f"http://{self.catalogIP}:{self.catalogPort}/getLink", json=data)
            link = json.loads(r.text)
            print("link: ", link)
...
if __name__=='__main__':
    ...
    ip = config['ip']
    port = config['port']
    print("connesso al seguente ip: ", ip)
