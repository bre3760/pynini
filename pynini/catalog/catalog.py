            ...
            try:
                with open('catalog.json', 'r+') as f:
                    ...
                    if found is False:
                        ...
                        # c'è bisogno di dare un errore se non troviamo 
                        # il dispositivo se già non c'è?
        ...
        if len(uri) == 1 and uri[0] == 'setThreshold':
            try:
                with open('catalog.json', 'r+') as f:
                    ...
                    bread_type = new_th.pop('name')
                    if bread_type in catalog['thresholds'] :
                        catalog['thresholds'].pop(bread_type)

                    
                    catalog['thresholds'][bread_type] = new_th

                    f.seek(0)
                    f.write(json.dumps(catalog, indent=4, sort_keys=True))
                    f.truncate()
                    f.close()
                    print(f'${bread_type} bread - added to the catalog')
                    return 'catalog file successfully written'
            except KeyError:
                raise cherrypy.HTTPError(404, 'The catalog file was not found')
