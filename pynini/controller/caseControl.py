...
global CATEGORY
CATALOG_ADDRESS = "http://localhost:9100" # deciso che sarà una variabile globale, accessibile da tutti gli script di tutto il progetto
...
class CaseControl(object):
    ...
    def getMaxTemperatureThreshold(self):
        try:
            threshold_URL = "http://" + self.IP_catalog + ":" + str(self.port_catalog) + "/thresholds"
            r = requests.get(threshold_URL)
            print("Maximum temperature allowed by " + self.clientID)
            threshold = r.text

            obj = json.loads(threshold)
            res = next((sub for sub in obj if sub['type'] == self.type), None)
            maxTemperature = res["max_temperature_th"]
    ...
    def getMinTemperatureThreshold(self):
        ...
        try:
            ...
            obj = json.loads(threshold)
            res = next((sub for sub in obj if sub['type'] == self.type), None)
            minTemperature = res["min_temperature_th"]
    ...
    def getMaxHumidityThreshold(self):
        try:
            ...
            obj = json.loads(threshold)
            res = next((sub for sub in obj if sub['type'] == self.type), None)
            maxHumidity = res["max_humidity_th"]
    ...
    def getMinHumidityThreshold(self):
        try:
            ...
            obj = json.loads(threshold)
            res = next((sub for sub in obj if sub['type'] == self.type), None)
            minHumidity = res["min_humidity_th"]
    ...
    def getBreadType(self):
        try:
            case_URL = "http://" + str(self.IP_catalog) + ":" + str(self.port_catalog) + "/cases"
            r = requests.get(case_URL)
            print("Bread in " + self.clientID)
            case_txt = r.text

            obj = json.loads(case_txt)

            res = next((sub for sub in obj if sub['case_ID'] == self.clientID), None)
            name = res["bread_type"]
            print(name)
